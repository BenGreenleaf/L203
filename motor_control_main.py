...
def update_mode(state, mode):
    if state == (1,0,1,0):
        return "forward"
    elif state == (0,1,0,1): 
        return "right turn" # strict right turning
    elif state == (1,1,1,0):
        return "right turn decision" # t junction type turning - needs a decision made based on whether blocks are being held or not
    elif state == (0,0,1,1):
        return "left turn"
    elif state == (1,0,1,1):
        return "left turn decision"
    elif state == (0,1,1,1):
